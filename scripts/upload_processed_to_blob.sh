...
# Upload a local processed shard run to Azure Blob Storage.
#
# Blob layout:
#   processed/<run_id>/...
...
if [ ! -d "$RUN_DIR" ]; then
  echo "Processed run directory not found: $RUN_DIR"
  exit 1
fi

if [ ! -f "$RUN_DIR/manifest.json" ]; then
  echo "Processed manifest not found: $RUN_DIR/manifest.json"
  exit 1
fi
...
echo "latest pointer: $CONTAINER/processed/latest.json"
