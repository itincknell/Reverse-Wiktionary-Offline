...
# Download a processed shard run from Azure Blob Storage.
#
# By default this reads processed/latest.json and downloads the run it points to.
...
echo "=== Downloading Processed Run ==="
echo "run id: $RUN_ID"
...
if [ ! -f "$RUN_DIR/manifest.json" ]; then
  echo "Downloaded run is missing manifest: $RUN_DIR/manifest.json"
  exit 1
fi
